...
function getNode(llist, positionFromTail) {
        ...
        //now we need to traverse the list
        current= llist
        for(let i=0; i< count-positionFromTail-1; i++){// -1 because we want to fall in the position behind the desired one so we set the current to fall in the current.next
            current= current.next
        }
