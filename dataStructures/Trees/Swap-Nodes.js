...
function swapNodes(indexes, queries) {
    const iterations = indexes.length;
    const tree = Array(iterations + 1).fill(null).map(() => ({ left: -1, right: -1 })); // Here we create the tree from the indexes

    for (let i = 0; i < iterations; i++) {
        tree[i + 1].left = indexes[i][0]; // here we use i+1 to not touch the first element which is the root we set the a and b elements a been the left child  
        tree[i + 1].right = indexes[i][1]; //b been the right
    }

    const inOrderTraversal = (node, result) => { // here we perform the inorder traversal
        if (node === -1) return;
    ...
    for (const k of queries) {
        for (let depth = 1, nodes = [1]; nodes.length > 0; depth++) {
            const nextLevel = [];
            for (const node of nodes) {
                if (depth % k === 0) { //if depth is a multiple of key swap the nodes
                    [tree[node].left, tree[node].right] = [tree[node].right, tree[node].left];
                }
                if (tree[node].left !== -1) nextLevel.push(tree[node].left);
                if (tree[node].right !== -1) nextLevel.push(tree[node].right);
            }
            nodes = nextLevel;
        }
        const result = [];
